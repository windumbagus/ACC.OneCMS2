...
class SeamlessParamController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($Hasilsrole->OUT_DATA == 'Super Admin' || $Hasilsrole->OUT_DATA == 'Super_Admin' || $Hasilsrole->OUT_DATA == 'seamless')
        {
            return view(
                'seamless_param',[
                   // 'Role' => $Hasils->Role,
                    'SeamlessParams'=>$Hasils->OUT_DATA,
                   // 'Roles'=>$Hasils2->Roles,
    ...
    public function delete(Request $request)
    {
        ...
        $data = json_encode(array(
            "doSendParamSimulation" => array(   
                "TRANSACTION_CODE"=>"DELETE_PARAM_SIMULATION",
                "P_LANGUAGE"=>"IN",
        ...
        if ($Hasils->OUT_STAT == "T"){
            return redirect("seamless-param")->with('success','Param Simulation berhasil terhapus');
        }else{
            return redirect("seamless-param")->with('error',$Hasils->OUT_MESS);
        }
