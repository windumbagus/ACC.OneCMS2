...
class SeamlessDataLeadsDetailController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       // $bulantahun = date('m/Y', strtotime(now()));
        ...
        $data = json_encode(array(
            "doSendDataCMS" => array(   
                "TRANSACTION_CODE"=>"TRACKING_LEADS",
                "PNO_REGISTRASI"=>$request->leadsid,
            ),
        ));
        //dd($data);
        ...
        if ($Hasilsrole->OUT_DATA == 'Super Admin' || $Hasilsrole->OUT_DATA == 'Super_Admin' || $Hasilsrole->OUT_DATA == 'seamless')
        {
             // dd($Hasils);
             return view(
                'seamless_dataleads_detail',[
                   // 'Role' => $Hasils->Role,
                    'SeamlessDataLeadsDetail'=>$Hasils->OUT_DATA,
                   // 'Roles'=>$Hasils2->Roles,
                  //  'UserCategories'=>$Hasils2->UserCategory, 
                  'role'=> $Hasilsrole->OUT_DATA,
                  'leadsid'=>$request->leadsid,
